...
namespace psychicui {
    ...
    Match Rule::matches(const Component *component) const {
        ...
        if (!_tag.empty()) {
            ...
            if (std::find(component->tags().begin(), component->tags().end(), _tag) == component->tags().end()) {
                ...
                return parent && matches(parent) != unmatched ? inherited : unmatched;
            }
        }
        ...
        switch (_pseudo) {
            case hover:
                if (!component->mouseOver()) { return parent && matches(parent) != unmatched ? inherited : unmatched; }
                break;
            case active:
                if (!component->mouseDown()) { return parent && matches(parent) != unmatched ? inherited : unmatched; }
                break;
            default:
                break;
        }
        ...
        if (_next) {
            ...
            return _next->matches(parent) != unmatched ? direct : unmatched;
        }
